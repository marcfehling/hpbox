...
#ifndef solver_cg_gmg_h
#define solver_cg_gmg_h
    ...
    public:
        ...
        Args &&...operator_args)
      {
        ...
        if (mg_data.transfer.perform_h_transfer)
          coarse_grid_triangulations = MGTransferGlobalCoarseningTools::
        ...
        const auto get_max_active_fe_degree = [&](const auto &dof_handler) {
          ...
          for (auto &cell : dof_handler.active_cell_iterators())
            if (cell->is_locally_owned())
              max =
                std::max(max,
                         dof_handler.get_fe(cell->active_fe_index()).degree);
        ...
        const unsigned int n_p_levels =
          MGTransferGlobalCoarseningTools::
            create_polynomial_coarsening_sequence(get_max_active_fe_degree(
                                                    dof_handler),
        ...
        for (unsigned int i = 0; i < dof_handler.get_fe_collection().size();
             ++i)
          {
            ...
            Assert(fe_index_for_degree.find(degree) ==
                    ...
                    if (cell->is_locally_owned())
                      {
                        ...
                        cell->set_active_fe_index(
                          fe_index_for_degree[next_degree]);
                      }
                    cell_other++;
                  }
              }
        ...
        for (unsigned int level = minlevel; level < minlevel_p; ++level)
          transfers[level + 1].reinit_geometric_transfer(
        ...
        for (unsigned int level = minlevel_p; level < maxlevel; ++level)
          transfers[level + 1].reinit_polynomial_transfer(
            dof_handlers[level + 1],
            dof_handlers[level],
            constraints[level + 1],
            constraints[level]);
